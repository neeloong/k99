...
function setHandleWithMethod(router: Router, path: string, item: any) {
...
function setHandleItem(router: Router, path: string, item: any): void {
	if (!item) { return; }
...
function createRegister(
	run: (router: Router, value: any) => void,
	test: (v: any) => boolean,
	onlyDefault?: boolean
) {
	return async function(file: Scanner.FileItem, router: Router): Promise<boolean> {
		try {
